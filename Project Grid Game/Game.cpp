...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
Game::Game() : m_nGridRows(k_nGridRowsDefault), m_nGridColumns(k_nGridColumnsDefault) /* ?? add initializers for member variables m_iRowBoat, m_iColumnBoat, and m_nTorpedoes. ?? */
{
	m_iRowBoat = 0;
    m_iColumnBoat = 0;
...
void Game::initializeGrid()
{
	...
	{
		for (int iColumn = 0; iColumn < m_nGridRows; iColumn++)
		{
...
bool Game::isOnTarget(int iRow, int iColumn)
{
	...
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	if ((iRow == m_iRowBoat) && (iColumn == m_iColumnBoat))
	{
		bHit = true;
	}
...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
	...
	// This corresponds to iRow and iColumn being in one of the eight grid squares surrounding the
	...
	// the target boat. 
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	if (((iRow == m_iRowBoat) || (iRow - 1 == m_iRowBoat) || (iRow + 1 == m_iRowBoat)) &&
        ((iColumn == m_iColumnBoat) || (iColumn - 1 == m_iColumnBoat) || (iColumn + 1 == m_iColumnBoat)))
	{
		bClose = true;
	}
...
std::tuple<int, int> Game::promptForGridCoord()
{
	...
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
        /* ?? Prompt user using std::cout for row ?? */
        std::cout << "Please enter grid row number: " << std::endl;
	/* ?? Use std::cin ?? */
    std::cin >> iRow;

        /* ?? Prompt user using std::cout for column ?? */
...
void Game::play()
{
	...
	while ((!bSunk) && (this->m_nTorpedoes > 0))
	{
		...
		if ((m_nTorpedoes == 0) && (!bSunk))
		{
			...
			/* ?? call member function to display the grid and show the location of the battle boat ?? */
            displayGrid(true);
